 ...
 *
 * Manages:
 * - notifications[] (max 50, newest first, deduplicated)
 * - unreadCount (computed from notifications)
 * - connectionStatus ('connected' | 'reconnecting' | 'polling' | 'disconnected')
 * - addNotification (caps at 50, deduplicates by ID)
 * - markAsRead (updates unread count within 1 second per req 12.4)
 * - markAllAsRead
 * - setConnectionStatus
 *
